اینجا می‌خوام نحوه استفاده از `cURL` در Command Line رو توضیح بدم، `cURL` یک ابزار قوی برای ارسال درخواست‌های HTTP و FTP است. شما میتونین با استفاده از اون فایل‌ها رو دانلود یا آپلود کنین، درخواست‌های GET و POST ارسال کنین و بسیاری از عملیات دیگه شبکه رو انجام بدین.

### 1. نصب cURL:

- در اکثر سیستم‌های Unix-based مثل لینوکس و macOS، `cURL` از قبل نصب شده. برای بررسی، دستور زیر رو اجرا کنین:
```bash
curl --version
```

- در ویندوز، می‌تونین با دانلود فایل نصبیش از سایت رسمی [cURL](https://curl.se/download.html)اونو  نصب کنین.
